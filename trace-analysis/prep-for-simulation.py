...
if(log_pos=="client") :
  ...
  if(session_number != log_session) :
    ...
    output_handle=open("downlink-"+str(log_time)+"-"+str(session_number)+".logs","w");
elif(log_pos=="server") :
  output_handle=open("uplink-"+str(log_time)+"-"+str(session_number)+".logs","w");
...
for line in fh.readlines() :
   ...
   if(fields[0]=="OUTGOING") : 
      ...
      if(sender_id==session_number) :
        ...
        output_handle.write(str(recv_time_ms-start_time)+"\t1400\n");
