...
if [  $# -lt 5 ] ; then
   echo "Usage: ./gen-pulse-train.sh numPulses  pulsewidth pulse separation start_freq noise_bw"
   exit
fi
num_pulses=$1
pulse_width=$2
...
while [ $i -lt $num_pulses ] ; do 
    ...
    ./tone-gen -c $freq -b $noise_bw -d $pulse_width -a 1
    mv noiseShifted.dat $freq.dat
    # generate silence 
    ./tone-gen -c $freq -b $noise_bw -d $silence_time -a 0
