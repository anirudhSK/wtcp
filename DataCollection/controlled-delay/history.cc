...
void History::active_purge( const uint32_t purge_seq  ) {

  assert(purge_seq > _max_rx_seq_no) ;  
  _max_rx_seq_no=purge_seq;
  ...
  /* because this is an active purge and purge_seq > max_rx_seq_no  */
  assert(new_size==0);  
  /* This is the number of packets rx between x >= _next_exp_seq_no + 1  and x <= purge_seq */
  uint32_t pkts_rx_before_purge_seq=old_size-new_size;
